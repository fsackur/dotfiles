...
# source: /usr/include/linux/input-event-codes.h (codes up to 0x2e7) and custom ratbag codes (-1, 1073741824..)
enum Key {
    # KEY_RESERVED = 0
    KEY_ESC = 1
    KEY_1 = 2
    ...
    KEY_KPCOMMA = 121
    KEY_HANGEUL = 122
    KEY_HANGUEL = 122  # alias to KEY_HANGEUL
    KEY_HANJA = 123
    KEY_YEN = 124
    KEY_LEFTMETA = 125
    KEY_RIGHTMETA = 126
    ...
    KEY_SCREENLOCK = 152  # alias for KEY_COFFEE
    KEY_ROTATE_DISPLAY = 153  <# Display orientation for e.g. tablets #>
    KEY_DIRECTION = 153  # alias to KEY_ROTATE_DISPLAY
    ...
    KEY_DASHBOARD = 204  # alias to KEY_ALL_APPLICATIONS
    KEY_SUSPEND = 205
...
<# Code 255 is reserved for special needs of AT keyboard driver #>
    ...
    BTN_GAMEPAD = 0x130
    BTN_SOUTH = 0x130
    BTN_A = 0x130  # alias to BTN_SOUTH
    BTN_EAST = 0x131
    BTN_B = 0x131  # alias to BTN_EAST
    BTN_C = 0x132
    BTN_NORTH = 0x133
    BTN_X = 0x133  # alias to BTN_NORTH
    BTN_WEST = 0x134
    BTN_Y = 0x134  # alias to BTN_WEST
    BTN_Z = 0x135
    BTN_TL = 0x136
    BTN_TR = 0x137
    BTN_TL2 = 0x138
    ...
    KEY_ZOOM = 0x174  # alias to KEY_FULL_SCREEN
    KEY_MODE = 0x175
    KEY_KEYBOARD = 0x176
    KEY_ASPECT_RATIO = 0x177  <# HUTRR37: Aspect #>
    KEY_SCREEN = 0x177  # alias to KEY_ASPECT_RATIO
    ...
    KEY_BRIGHTNESS_TOGGLE = 0x1af  # alias to KEY_DISPLAYTOGGLE
 ...
 * of the eprivacy screen at boot.
 #>
    ...
    KEY_ACTION_ON_SELECTION = 0x254  <# AL Action on Selection (HUTRR119) #>
    KEY_CONTEXTUAL_INSERT = 0x255  <# AL Contextual Insertion (HUTRR119) #>
    KEY_CONTEXTUAL_QUERY = 0x256  <# AL Contextual Query (HUTRR119) #>
...
<# Performance Boost key (Alienware)/G-Mode key (Dell) #>
    ...
    # ratbag-specific special codes
    INVALID = -1
    UNKNOWN = 1 -shl 30
    DOUBLECLICK = (1 -shl 30) + 1
    WHEEL_LEFT = (1 -shl 30) + 2
    WHEEL_RIGHT = (1 -shl 30) + 3
    WHEEL_UP = (1 -shl 30) + 4
    WHEEL_DOWN = (1 -shl 30) + 5
    RATCHET_MODE_SWITCH = (1 -shl 30) + 6
    RESOLUTION_CYCLE_UP = (1 -shl 30) + 7
    RESOLUTION_CYCLE_DOWN = (1 -shl 30) + 8
    RESOLUTION_UP = (1 -shl 30) + 9
    RESOLUTION_DOWN = (1 -shl 30) + 10
    RESOLUTION_ALTERNATE = (1 -shl 30) + 11
    RESOLUTION_DEFAULT = (1 -shl 30) + 12
    PROFILE_CYCLE_UP = (1 -shl 30) + 13
    PROFILE_CYCLE_DOWN = (1 -shl 30) + 14
    PROFILE_UP = (1 -shl 30) + 15
    PROFILE_DOWN = (1 -shl 30) + 16
    SECOND_MODE = (1 -shl 30) + 17
    BATTERY_LEVEL = (1 -shl 30) + 18
}

[Key[]]$RatbagButtons = [Key]::BTN_1..[Key]::BTN_9
[Key[]]$RatbagSpecials = [Key]::UNKNOWN..[Key]::BATTERY_LEVEL

$Script:ErrorActionPreference = "Stop"

$Script:C18Name = "SOAI USB Gaming Mouse"
...
function Install-ZelotesDriver {
    <#
        .DESCRIPTION
        Piper is the GUI. But it doesn't show all buttons perfectly - perhaps due to missing SVG?

        .LINK
        https://github.com/libratbag/libratbag/pull/1877/changes
    #>

    [CmdletBinding()]
    param ()

    Push-Location $env:GITROOT
    try {
        git clone ssh://github.com/libratbag/libratbag --origin upstream
        cd libratbag
        git remote add PadreAdamo ssh://github.com/PadreAdamo/libratbag
        git fetch PadreAdamo
        git switch soai-c18-driver

        sudo dnf install piper libevdev-devel libunistring-devel check-devel valgrind python3-devel swig
        meson builddir
        ninja -C builddir
    ...
    $Override = "[Service]`nExecStart=`nExecStart=/usr/local/bin/ratbagd"
    sudo mkdir -p /etc/systemd/system/ratbagd.service.d
    $Override | sudo tee /etc/systemd/system/ratbagd.service.d/override.conf | Out-Null
    sudo systemctl restart ratbagd
}

function Get-ZelotesProfileCount {
    [CmdletBinding()]
    param ()

    [string]$Count = (ratbagctl $C18Name info) -match "^\s*Number of Profiles: " -replace ".*:\s*"
    [int]$Count
}

function Get-ZelotesActiveProfile {
    ratbagctl $C18Name profile active get
}

function Get-Zelotes {
    [CmdletBinding()]
    param (
        [Parameter(Position = 0)]
        [ValidateSet(0, 1, "active")]
        [string]$Profile = "active"
    )

    if ($Profile -eq "active") {$Profile = Get-ZelotesActiveProfile}

    $Output = ratbagctl $C18Name profile $Profile get
    if (!$?) {
        throw "$Output"
    }
    ...
    $ButtonLines = $Output -match "^Button"
    $ButtonLines | % {
        if ($_ -notmatch "^Button:\s+(?<Button>\d+) is mapped to( key)? '(?<Key>.*)'") {
            Write-Error "Failed to parse: '$_'"
            return
        }
        $i = [int]$Matches.Button
        $ButtonName = $Script:ButtonNames[$i]

        $Key = $Matches.Key -replace "-", "_"
        if ($Key -match "button (?<Key>\d+)$") {
            $Key = $ButtonAliases[([int]$Matches.Key)]
        }

        $Mapping[$ButtonName] = [Key]$Key
    }
...
function Set-Zelotes {
    ...
    # $c.Parameters.Values | ? ParameterType -eq ([Key])
    $Mappings = $PSBoundParameters.GetEnumerator() | ? Value -is ([Key])
    $Mappings | % {
        $Button = $ButtonNames.IndexOf($_.Key)
        $Value = $_.Value

        $ValArgs = if ($Value -in $RatbagButtons) {
            "button", $Value -replace ".*_"
        } elseif ($Value -in $RatbagSpecials) {
            "special", ($Value -replace "_", "-").ToLower()
        } else {
            "key", $Value
        }

        ratbagctl $C18Name profile $Profile button $Button action set @ValArgs
    }
}
